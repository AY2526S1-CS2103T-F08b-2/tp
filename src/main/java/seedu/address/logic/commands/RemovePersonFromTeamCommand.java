...
public class RemovePersonFromTeamCommand extends Command {
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        // Check if person is actually in this team by checking the person's teams
        boolean isInTeam = personToRemove.getTeams().stream()
                .anyMatch(team -> team.getTeamName().equals(teamName));

        if (!isInTeam) {
            logger.log(Level.WARNING, "Person {0} is not in team {1}", new Object[] {Messages.format(personToRemove), teamName});
            throw new CommandException(
                    String.format(MESSAGE_PERSON_NOT_IN_TEAM,
                            Messages.format(personToRemove), teamName));
        }

        // Use the model's relationship management to remove person from team
        ...
        logger.info("Removed person " + Messages.format(personToRemove) + " from team " + teamName
                + ". Updated team: " + Messages.format(updatedTeam));
